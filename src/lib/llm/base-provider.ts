 ...
 */
  ...
  protected generateCustomFinalPrompt(text: string, customPrompt: string): string {
    // デバッグのために入力内容をログ出力
    console.log('生成前のカスタムプロンプト (先頭部分):', customPrompt.substring(0, 100) + '...');
    console.log('処理するテキスト内容の一部:', text.substring(0, 100) + '...');
    ...
    if (customPrompt.includes('## コンテンツ')) {
      ...
      const finalPrompt = customPrompt.replace(contentPlaceholder, text);
      console.log('実行時のプロンプト(先頭部分):', finalPrompt.substring(0, 150) + '...');
      return finalPrompt;
    } else {
...
`;
  ...
  public generateDebugPrompt(text: string, settings: AppSettings): string {
    ...
    console.log('デバッグプロンプト生成時のスタイル:', settings.basic.designStyle, styleDefinition.displayName);
